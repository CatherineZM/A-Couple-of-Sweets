...
export default function ProductCard(props: Props): JSX.Element {
    ...
    useEffect(() => {
        if (open && swiperInst) {
            // Let the modal render & size, then update Swiper measurements
            requestAnimationFrame(() => {
                try {
                    swiperInst.update();
                    // Ensure first slide is in view after update
                    if (typeof swiperInst.slideTo === "function") {
                        swiperInst.slideTo(0, 0);
                    }
                } catch {}
            });
        }
